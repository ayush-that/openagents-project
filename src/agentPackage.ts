...
export const starterAgent: AgentDraft = {
  ...
  soul: "You are 0G Research Scout, a focused research assistant. You prefer concise answers, cite evidence, and write durable memories after important findings.",
  memory:
  ...
  workflow: [
    ...
    },
    {
      ...
      instruction:
        "Append durable decisions to MEMORY.md and publish package/log roots to 0G Storage.",
    },
  ],
  storage: {
...
function createSkillExportEntries(skills: SkillDraft[]) {
  ...
  return skills
    ...
    .map((skill) => {
      const baseSegment = normalizeSkillPathSegment(
        skill.name,
        `skill-${normalizeSkillPathSegment(skill.id, "custom")}`,
      );
